...
import { SetCheckDate, actFetchSinglePropertyRequest } from "../../actions";
...
const SearchFilterInnerRoom = ({ catId, id }) => {
    ...
    if (!UserSelectedLocation && !dateCheckInOut.LocationId) {
      ...
      const respones = await dispatch(
        actFetchSinglePropertyRequest(
          id,
          DateCheckInOut?.arrivalDate,
          DateCheckInOut?.departureDate
        )
      );
      if (
        respones?.data?.room?.length ||
        respones?.status == 200 ||
        respones?.status == 201
      ) {
        dispatch(SetCheckDate(DateCheckInOut));
          ...
          <div className="col-lg-3 col-md-6 colBottomSpace">
            <div className="inputs-filed inputDateFields">
              ...
              <DatePicker
                ...
                minDate={new Date().getTime() + 2 * 24 * 60 * 60 * 1000}
                //# time range
...
export default SearchFilterInnerRoom;
